...
public class ProxyServiceImpl implements ProxyService {
	...
	public ResponseEntity<byte[]> proxyRouting(ProxyExchange<byte[]> proxy, HttpServletRequest request,
		...
		return switch (name) {
			...
			case "POST" -> postRouting(proxy, request, routedURI);
	...
	private ResponseEntity<byte[]> postRouting(ProxyExchange<byte[]> proxy, HttpServletRequest request, String routedURI) {
		log.debug(ROUTED_URI_MSG, routedURI);
		return proxy.uri(routedURI).post();
	}
